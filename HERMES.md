...
- Read or edit only allowlisted text files under `src/`, `public/images/`, and `docs/`.
...
3. Only transaction-managed, request-related text files under `src/`, `public/images/`, or `docs/`; all protected surfaces remain denied.
4. `git diff --check`, `npm ci`, `npm run build`, and `git diff --cached --check` succeed without changing the bounded diff.
